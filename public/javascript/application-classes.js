...
class Actions {
    ...
    search() {
        if (this.elements['searchBar']) {
            ...
            const searchBarService = new VoiceSearchService(RecognitionClass, false, this.elements['searchBar']);
 ...
 * @description Handles text insertion and editing for input elements and contenteditable fields.
 */
class TextInputHandler {
    /**
     * @constructor
     * @param {HTMLElement} element - The target element for text manipulation (input, textarea, or contenteditable).
     * @param {Logger} logger - Logger instance for debugging and error reporting.
     * @throws {Error} If no element is provided or if the element is not a valid input type.
     */    
    constructor(element, logger = new Logger()) {
        this.logger = logger;
        
        if (!element) {
            this.logger.error('No element provided');
            throw new Error('No element provided');
        }
        
        const validInputTypes = ['INPUT', 'TEXTAREA'];
        if (!validInputTypes.includes(element.tagName) && !element.isContentEditable) {
            this.logger.error('Invalid input element');
            throw new Error('Element must be an input, textarea, or contenteditable element');
        }
    ...
    /**
     * @method insertText
     * @description Inserts text into the element, respecting selection and cursor position.
     * @param {string} newText - The text to insert into the element.
     */    
    insertText(newText) {
        try {
            if (this.element.tagName === 'INPUT' || this.element.tagName === 'TEXTAREA') {
                this._handleInputText(newText);
            } else if (this.element.isContentEditable) {
                this._handleContentEditableText(newText);
            }
        } catch (error) {
            this.logger.error('Text insertion failed:', error);
        }
    }

    /**
     * @private
     * @method _handleInputText
     * @description Handles text insertion for input and textarea elements.
     * @param {string} newText - The text to insert.
     */    
    _handleInputText(newText) {
    ...
    /**
     * @private
     * @method _handleContentEditableText
     * @description Handles text insertion for contenteditable elements.
     * @param {string} newText - The text to insert.
     */    
    _handleContentEditableText(newText) {
...
class NoteService {
    ...
    constructor(indexedDB, logger = new Logger(false), VERSION_NUMBER = 1) {
        ...
        this.instanceSettings = new SetInstanceIDs(localStorage, (JSON.parse(localStorage.getItem(STORAGE_KEY)) || {}));
    ...
    _openDatabase() {
        return new Promise((resolve, reject) => {
            const dbRequest = this.indexedDB.open(`notesDatabase-${this.instanceID}`, this.VERSION_NUMBER);
...
async  saveToIndexDB(note) {
    ...
    async updateNote(noteId, changes) {
        try {
            ...
            request.onerror = (event) => {
                this.logger.error(`Error fetching note for update:`, event.target.error);
            };
        } catch (error) {
    ...
    async deleteNote(id) {
        ...
        transaction.onerror = (event) => {
            this.logger.error(`Transaction error while deleting note: ${event.target.error}`);
        };

        transaction.onabort = (event) => {
            this.logger.error(`Transaction aborted while deleting note: ${event.target.error}`);
        };
    }
    ...
    addDeletedNoteId(id) {
        ...
        let deletedNotes = JSON.parse(localStorage.getItem(this.DELETED_NOTES_KEY)) || [];
...
async  getSingleNote(noteID, callback){
        ...
        request.onerror = (event) => {
            this.logger.log(`An error occured getting note with ID : ${noteID}, error:${event.target.error}`);
        }
    }
...
async updateSync(processedNotes) {
        ...
        for (const processedNote of processedNotes) {
            ...
            request.onsuccess = (event) => {
                ...
                if (note && !note.synced) {
                    ...
                    this.logger.log(`Note with ID ${processedNote.id} not found or already synced.`);
                }
            };
    ...
    async fetchBackendData() {
        try {
            const response = await fetch(`/api/fetchnotes?user_id=${this.instanceID}`, { method: 'GET' });
            if (!response.ok) {
            ...
            transaction.onerror = (event) => {
                this.logger.error(`Error syncing backend data: ${event.target.error}`);
            };
        } catch (error) {
            this.logger.error(`An error occurred during fetchBackendData: ${error.message}`);
        }
    }
...
class SetInstanceIDs {
    ...
    setInstance(name, Id) {
        ...
        if (this.instances.has(name) && this.instances.get(name) === Id) {
            this.logger.log(`Instance "${name}" already exists with the same ID. No update needed.`);
            return;
        }
